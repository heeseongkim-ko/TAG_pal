...
#ifdef  __TEIA__
/**
 * @brief Update SPI handler for deca driver
 * 
 * @details Updates the global SPI handler pointer and pin configurations.
 *          This allows external drivers to inject their own SPI handler.
 * 
 * @param handler Pointer to SPI handler structure
 * @param cs_pin Chip select pin number
 * @param irq_pin IRQ pin number
 */
void update_spi_handler(spi_handle_t *handler, uint16_t cs_pin, uint16_t irq_pin)
{
	pgSpiHandler = handler;
